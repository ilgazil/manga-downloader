// @ts-ignore
import HTMLElementData from './htmlelement.ts';
// @ts-ignore
import {RegExpObject, Pattern, tag} from './types.ts';
...
class BeautifulDom {
    ...
    private getByAttribute(attribute : string, attributeValue?:string): HTMLElementData[] {
        ...
        this.parsedData.forEach((parsedData) => {
            let openingTag : string = parsedData['outerHTML']
                .slice(parsedData['outerHTML']
                    .indexOf('<'),parsedData['outerHTML']
                    .indexOf('>') + 1 );

            if(openingTag.match(attributeRegExp)){
                matchingTags.push(parsedData);
            }
        });
        this.parsedData = matchingTags;
        return matchingTags;
